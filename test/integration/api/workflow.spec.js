...
// DEPRECATED: TODO: remove this test after migration to public lodash get method
test('logs helpful error message when misusing helper', async (t) => {
  const authorizationHeaders = await getAccessTokenHeaders({ t,
    permissions: [
      'workflow:create:all',
      'workflow:edit:all',
  ...
  const workflowName = 'Helper Misuse Workflow'
  const newUsername = 'Workflow Helper Misuse User'
  const computedWithoutQuotes = 'computed'
  ...
  const { body: workflow } = await request(t.context.serverUrl)
    ...
    .send({
      ...
      event: 'user__created',
      computed: {
        date: 'new Date(new Date().getTime() + (72 * 60 * 60 * 1000)).toISOString()',
        isCurrentTest: `_.get(user, "metadata.workflowName") === '${workflowName}'`
      },
      run: [{
        filter: 'computed.isCurrentTest && !_.get(user, "metadata.salesCallDate")',
        endpointMethod: 'PATCH',
        endpointUri: '/users/${user.id}',
        endpointPayload: {
          metadata: {
            salesCallDate: `!!S.get(${computedWithoutQuotes}, "date")`
          }
        }
      },
      {
  ...
  const { body: userAfterError } = await request(t.context.serverUrl)
    .get(`/users/${user.id}`)
    .set(authorizationHeaders)
    .expect(200)

  t.is(userAfterError.username, newUsername)
  t.deepEqual(userAfterError.metadata, { workflowName })

  const { body: workflowWithError } = await request(t.context.serverUrl)
    .get(`/workflows/${workflow.id}?logs=`)
    .set(authorizationHeaders)
    .expect(200)

  // Exclude any log due to workflow events from concurrent tests
  const erroneousWorkflowLogs = workflowWithError.logs.filter(
    log => log.metadata.eventObjectId === user.id
  )
  t.is(erroneousWorkflowLogs.length, 1)

  let lastErrorLog = erroneousWorkflowLogs[0]
  t.is(lastErrorLog.type, 'runError')
  t.is(lastErrorLog.statusCode, 422)
  t.true(lastErrorLog.step.error)
  t.regex(lastErrorLog.metadata.message, /string object name/i)
  t.regex(lastErrorLog.metadata.message, /first argument/i)
  t.is(lastErrorLog.metadata.endpointMethod, 'PATCH')
  t.is(lastErrorLog.metadata.eventObjectId, user.id)
  t.deepEqual(Object.keys(lastErrorLog.metadata.endpointPayload).length, 1)
  t.falsy(lastErrorLog.metadata.endpointPayload.metadata.salesCallDate) // not updated

  const workflowWithErrorLogs = erroneousWorkflowLogs.filter(log => log.step.error)
  // no successful action
  t.is(workflowWithErrorLogs.length, erroneousWorkflowLogs.length)

  // Fixing error
  const runWithPayloadAfterFix = _.cloneDeep(workflow.run)
  runWithPayloadAfterFix[0].endpointPayload.metadata.salesCallDate = `!!S.get("${
    computedWithoutQuotes
  }", "date")`
  ...
  await request(t.context.serverUrl)
    ...
    .set(authorizationHeaders)
    .send({
      run: runWithPayloadAfterFix
    })

  // Trigger Workflow
  const { body: userCreatedAfterFix } = await request(t.context.serverUrl)
    .post(`/users`)
    .set(authorizationHeaders)
    .send({
      username: `${newUsername} - After Fix`,
      password: 'randomPassword',
      metadata: { workflowName }
    })
    .expect(200)
  ...
  const { body: userAfterFix } = await request(t.context.serverUrl)
    .get(`/users/${userCreatedAfterFix.id}`)
    .set(authorizationHeaders)
    .expect(200)

  t.is(userAfterFix.username, `${newUsername} - After Fix`)
  t.truthy(userAfterFix.metadata.salesCallDate)

  const { body: workflowAfterFix } = await request(t.context.serverUrl)
    .get(`/workflows/${workflow.id}?logs=`)
    .set(authorizationHeaders)
    .expect(200)

  const afterFixLogs = workflowAfterFix.logs.filter(
    log => [user.id, userCreatedAfterFix.id].includes(log.metadata.eventObjectId)
  )
  const workflowAfterFixErrorLogs = afterFixLogs.filter(log => log.step.error)
  const workflowAfterFixActions = afterFixLogs.filter(
    log => log.type === 'action' && !log.step.error && !log.step.stopped
  )

  // No additional errors
  t.deepEqual(workflowWithErrorLogs, workflowAfterFixErrorLogs)
  t.is(workflowAfterFixActions.length, workflow.run.length)
})
// DEPRECATED:END
