const { computeDate, getPureDate } = require('./time')
const ms = require('ms')

const retentionLogDuration = '31 days'

function getRetentionLimitDate () {
  // round to pure date (0h0m0s) to have a relative stable value to display if there is any error value
  ...
  const today = getPureDate(new Date().toISOString())
